...
import { sendMessage, getSessionDetails } from '../api/client';
import { ConversationMessage } from '../types';

// Get a standalone t() that reads from current language without React context
// This is needed because the hook runs inside components but we don't want to add useI18n dependency
function getT(): (key: string, vars?: Record<string, string | number>) => string {
  try {
    // Try to read lang from localStorage (same source as I18nProvider)
    const saved = localStorage.getItem('app_lang');
    const lang = saved === 'en' || saved === 'zh' ? saved : 'zh';
    // Dynamic import won't work here, so use a simple lookup approach
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (result.status === 'completed' && result.turn) {
        ...
        // 1. Show AI's natural language response FIRST
        ...
        // Add natural language response as the main content
        if (aiResponse) {
          aiContent.push(aiResponse);
        }

        // 2. Only show structured info if there are actual file/command operations
        if (hasSteps) {
          ...
          if (hasResult) {
            // Format the result into something readable
        ...
        if (aiContent.length === 0) {
          aiContent.push(t('structuredCompleted'));
        }
        ...
        try {
          ...
          setActiveSessionState(prev => prev ? {
            ...prev,
            turns: details.turns,
          } : null);
        } catch { /* ignore */ }
      ...
      const t = getT();
      const msg = err instanceof Error ? err.message : t('structuredRequestFailed');
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: '**' + t('structuredRequestFailed') + '**: ' + msg,
        timestamp: new Date().toISOString(),
        turn_num: 0,
      }]);
    }
